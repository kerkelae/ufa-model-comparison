"""μFA estimation using the gamma-distributed apparent diffusivities
assumption."""
...
@numba.jit()
def gamma_res_f(x, pa_data, bs):
    """Return residuals."""
    res = pa_data - gamma_model_f(x, bs)
    return res


def fit_gamma_model(pa_lte, pa_ste, bs, x0=None,
...
if __name__ == '__main__':  # Fit to phantom and volunteer data
    ...
    for s, r in zip(SUBJECT_DIRS, RESULTS_DIRS):
            ...
            for j in range(ys):
                for k in range(zs):
                    if mask[i, j, k]:
                        ...
                        x = fit_gamma_model(pa_lte, pa_ste, bs)
                        S0[i, j, k] = x[0]
                        MD[i, j, k] = x[1]
                        V_iso[i, j, k] = x[2]
                        V_aniso[i, j, k] = x[3]
        ...
        nib.save(nib.Nifti1Image(S0, affine),
                 os.path.join(r, 'gam_S0.nii.gz'))
        nib.save(nib.Nifti1Image(MD, affine),
                 os.path.join(r, 'gam_MD.nii.gz'))
        nib.save(nib.Nifti1Image(V_iso, affine),
                 os.path.join(r, 'gam_V_iso.nii.gz'))
        nib.save(nib.Nifti1Image(V_aniso, affine),
                 os.path.join(r, 'gam_V_aniso.nii.gz'))
        nib.save(nib.Nifti1Image(uFA, affine),
                 os.path.join(r, 'gam_uFA.nii.gz'))
