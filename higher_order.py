"""μFA estimation using the 3rd order signal cumulant expansion."""
...
@numba.jit()
def ho_res_f(x, PA_data, bs):
    """Return residuals."""
    res = PA_data - ho_model_f(x, bs)
    return res


def fit_ho_model(pa_lte, pa_ste, bs, x0=None,
                 bounds=([0, 0, 0, 0, 0],
...
if __name__ == '__main__':  # Fit to phantom and volunteer data
    ...
    for s, r in zip(SUBJECT_DIRS, RESULTS_DIRS):
            ...
            for j in range(ys):
                for k in range(zs):
                    if mask[i, j, k]:
                        ...
                        x = fit_ho_model(pa_lte, pa_ste, bs)
                        S0[i, j, k] = x[0]
                        MD[i, j, k] = x[1]
                        V_iso[i, j, k] = x[2]
                        V_aniso[i, j, k] = x[3]
        ...
        nib.save(nib.Nifti1Image(S0, affine),
                 os.path.join(r, 'ho_S0.nii.gz'))
        nib.save(nib.Nifti1Image(MD, affine),
                 os.path.join(r, 'ho_MD.nii.gz'))
        nib.save(nib.Nifti1Image(V_iso, affine),
                 os.path.join(r, 'ho_V_iso.nii.gz'))
        nib.save(nib.Nifti1Image(V_aniso, affine),
                 os.path.join(r, 'ho_V_aniso.nii.gz'))
        nib.save(nib.Nifti1Image(uFA, affine),
                 os.path.join(r, 'ho_uFA.nii.gz'))
        nib.save(nib.Nifti1Image(P3, affine),
                 os.path.join(r, 'ho_P3.nii.gz'))
